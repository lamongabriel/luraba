import { date, integer, index, pgTable, timestamp, uuid, varchar } from 'drizzle-orm/pg-core';
...
export const creditCardBudgetRecognitionsTable = pgTable(
  'credit_card_budget_recognitions',
  {
    ...
    purchaseId: uuid('purchase_id').notNull().references(() => creditCardPurchasesTable.id, { onDelete: 'cascade' }),
    installmentId: uuid('installment_id').references(() => creditCardInstallmentsTable.id, { onDelete: 'cascade' }),
  ...
  (table) => [
    index('credit_card_budget_recognitions_budget_month_idx').on(table.budgetMonth),
  ],
);
