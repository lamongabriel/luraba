...
import type { CreateHouseholdInput, CreateHouseholdInviteInput } from "@luraba/contracts";
...
export function HouseholdManagement({ householdId }: { householdId: string }) {
  ...
  if (query.isError || !household) {
    return (
      <ErrorState
        ...
        description={query.error?.message ?? "The household may no longer be available."}
  ...
  const countryName =
    locationOptionsQuery.data?.countries.find((country) => country.code === household.countryCode)
    ...
    <>
      <PageReveal className="min-w-0 max-w-full space-y-8">
        <PageHeader
          ...
          description={
            household.description || "A shared household workspace for organizing finances."
          }
          backHref="/households"
            ...
            <div className="flex items-center gap-1.5">
              ...
              {canManage ? (
                <Button onClick={() => setInviteOpen(true)}>
                  <HugeiconsIcon icon={Add01Icon} strokeWidth={2} /> Invite member
                </Button>
              ) : null}
            </div>
          }
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          <InfoItem label="Default currency" value={household.defaultCurrencyId} />
          <InfoItem label="Country" value={countryName} />
          <InfoItem label="Time zone" value={formatTimezoneLabel(household.timezone)} />
        </div>
        <section className="space-y-4">
          <SectionHeader title="Members" />
          <HouseholdMembersTable householdId={household.id} householdRole={household.role} />
        </section>
        <section className="space-y-4">
          <SectionHeader title="Outgoing invitations" />
          <HouseholdInvitationsTable householdId={household.id} householdRole={household.role} />
        </section>
      </PageReveal>
