...
export const creditCardInstallmentsTable = pgTable(
  'credit_card_installments',
  {
    ...
    billingCycleId: uuid('billing_cycle_id').notNull().references(() => creditCardBillingCyclesTable.id, { onDelete: 'cascade' }),
    installmentNumber: integer('installment_number').notNull(),
    amount: integer('amount').notNull(),
    createdAt: timestamp('created_at').notNull().defaultNow(),
  },
  (table) => [
    uniqueIndex('credit_card_installments_purchase_number_unique').on(table.purchaseId, table.installmentNumber),
  ],
);
