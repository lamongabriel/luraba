...
function checkPackages() {
  for (const [name, directory] of [
    ...
    requiredFile(join(root, directory, "pnpm-lock.yaml"), `${name} lockfile`);
  }
  if (existsSync(join(root, "pnpm-workspace.yaml")))
    errors.push("pnpm-workspace.yaml must be absent; packages are intentionally isolated.");
...
function checkEnvironment() {
  ...
  const apiValues = envFile(join(root, "api", ".env"));
  const uiPath = existsSync(join(root, "ui", ".env.local"))
    ? join(root, "ui", ".env.local")
    : join(root, "ui", ".env");
  requiredFile(join(root, ".env"), "root environment file (generated by pnpm run setup)");
  requiredFile(join(root, "api", ".env"), "API environment file (generated by pnpm run setup)");
  if (!existsSync(uiPath))
    errors.push("Missing UI environment file (ui/.env.local or ui/.env). Run pnpm run setup.");
...
function checkWorktree() {
  const result = spawnSync("git", ["status", "--porcelain", "--", "api", "ui"], {
    cwd: root,
    encoding: "utf8",
  });
