...
export function FilterSlider<TData>(props: FilterSliderProps<TData>) {
  ...
  const { value, range, unit, onValueChange } = React.useMemo(() => {
    ...
    return {
      value: props.value,
      range: props.range ?? (getIsValidRange(props.value) ? props.value : ([0, 100] as RangeValue)),
      unit: props.unit,
  ...
  const onFromInputChange = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!Number.isNaN(nextValue) && nextValue >= min && nextValue <= resolvedValue[1]) {
  ...
  const onToInputChange = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!Number.isNaN(nextValue) && nextValue <= max && nextValue >= resolvedValue[0]) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <div className="relative inline-flex">
        <PopoverTrigger asChild>
          <Button variant="outline" className={cn("border-dashed font-normal", value && "pr-8")}>
      ...
      <PopoverContent align="start" className="flex w-auto flex-col gap-4">
        ...
        <Button aria-label={`Clear ${title ?? "range"} filter`} variant="outline" onClick={onReset}>
          Clear
        </Button>
      </PopoverContent>
    </Popover>
