...
import { type AppClientError, getAppErrorDetails, toAppClientError } from "@/services/error-client";
...
export type AppMutationResult<TData, TVariables, TContext = unknown> = UseMutationResult<
  TData,
  AppClientError,
  TVariables,
  TContext
...
export function useAppMutation<TData, TVariables, TContext = unknown>(
  ...
  const { errorToast, fallbackMessage, onError, onSuccess, successToast, ...mutationOptions } =
    options ?? {};

  const resolvedFallbackMessage = fallbackMessage ?? definition.defaultErrorMessage;
