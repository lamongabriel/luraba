...
export function getNetWorthSummary(query: NetWorthQuery = {}): Promise<GetNetWorthSummaryResult> {
  return requestContract(netWorthEndpoints.summary, { query });
}

export function getNetWorthHistory(query: NetWorthQuery = {}): Promise<GetNetWorthHistoryResult> {
  return requestContract(netWorthEndpoints.history, { query });
}

export function getNetWorthAccounts(query: NetWorthQuery = {}): Promise<GetNetWorthAccountsResult> {
  return requestContract(netWorthEndpoints.accounts, { query });
}

export function getNetWorthCashFlow(query: NetWorthQuery = {}): Promise<GetNetWorthCashFlowResult> {
