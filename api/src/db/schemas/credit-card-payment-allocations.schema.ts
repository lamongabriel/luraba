...
export const creditCardPaymentAllocationsTable = pgTable('credit_card_payment_allocations', {
  ...
  paymentId: uuid('payment_id').notNull().references(() => creditCardPaymentsTable.id, { onDelete: 'cascade' }),
  billingCycleId: uuid('billing_cycle_id').references(() => creditCardBillingCyclesTable.id, { onDelete: 'cascade' }),
