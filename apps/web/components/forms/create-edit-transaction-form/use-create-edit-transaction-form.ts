...
export function useCreateEditTransactionForm({
  ...
  const onSubmit = form.handleSubmit((values) => {
    ...
    const amountMinor = majorToMinorUnits(
      ...
      getCurrencyPrecision(
        lookups,
        selectedCurrencyCode ?? transferFx.fromCurrencyCode,
      ),
    )
    ...
    if (values.kind === "transfer") {
      const body = {
        description: values.description,
        fromAccountId: values.fromAccountId,
        fromAmount: majorToMinorUnits(
          Number(values.amount),
          transferFx.fromPrecision,
        ),
        includeInBudget: values.includeInBudget,
        postedDate: values.postedDate,
        purchaseDate: values.purchaseDate,
        ...
        toAmount: majorToMinorUnits(
          Number(values.toAmount),
          transferFx.toPrecision,
        ),
      }
  ...
  return {
    errorMessage:
      mutations.map((mutation) => mutation.errorMessage).find(Boolean) ?? "",
    form,
