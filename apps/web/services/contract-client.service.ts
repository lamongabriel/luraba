...
export async function requestContract<T extends EndpointContract>(
  ...
  if (failure.success) {
    throw new AppClientError(failure.data.error.message, response.status, failure.data.error.code);
  }
  ...
  const envelope = apiSuccessResponseSchema(endpoint.response).safeParse(response.data);
  if (!envelope.success) {
    throw new AppClientError(
      `The API response did not match ${endpoint.method.toUpperCase()} ${endpoint.path}`,
      response.status,
