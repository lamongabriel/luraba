...
function isInlineEditable(row: TransactionFeedRow) {
  return row.rowKind !== "credit_card_payment" && row.originType !== "adjustment";
}
  ...
  return (
    <div className="min-w-36">
      <ComboboxControl
        ...
        onChange={(value) =>
          update({
            categoryId: value === UNCATEGORIZED_VALUE || value === row.categoryId ? null : value,
          })
        }
        disabled={isPending}
        ...
        renderOption={(option) => {
          const optionCategory = lookups.categories.find((item) => item.id === option.value);
