...
export function getTransactionFormKind(row?: TransactionFeedRow | null): TransactionFormKind {
...
export function getCurrencyPrecision(
  ...
  return lookups.currencies.find((currency) => currency.code === currencyCode)?.precision ?? 2;
}

export function getAccountCurrencyCode(lookups: TransactionLookups, accountId: string) {
  return lookups.accounts.find((account) => account.id === accountId)?.currencyCode;
}

export function getCardCurrencyCode(lookups: TransactionLookups, creditCardId: string) {
  return lookups.creditCards.find((card) => card.id === creditCardId)?.currencyCode;
}
...
export function getTransactionFormDefaultValues(
  ...
  const currencyCode =
    row?.currencyCode ??
    getCardCurrencyCode(lookups, purchase?.creditCardId ?? payment?.creditCardId ?? "");

  return {
    kind,
    description: purchase?.description ?? payment?.description ?? row?.description ?? "",
    ...
    creditCardId: purchase?.creditCardId ?? payment?.creditCardId ?? row?.creditCardId ?? "",
    ...
    purchaseDate: purchase?.purchaseDate ?? payment?.paymentDate ?? row?.purchaseDate ?? today,
    postedDate: purchase?.postedDate ?? payment?.postedDate ?? row?.postedDate ?? today,
    ...
    tagIds: purchase?.tags.map((tag) => tag.id) ?? row?.tags.map((tag) => tag.id) ?? [],
  };
}
