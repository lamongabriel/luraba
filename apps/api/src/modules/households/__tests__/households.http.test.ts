...
describe("households routes", () => {
  ...
  it("serves household roles, permissions, and invitation statuses from the API", async () => {
    const context = await createAuthenticatedContext();
    const headers = createAuthHeaders(context.token, context.household.id);

    const [roles, permissions, statuses] = await Promise.all([
      request(app).get("/api/v1/households/roles").set(headers),
      request(app).get("/api/v1/households/permissions").set(headers),
      request(app).get("/api/v1/households/invite-statuses").set(headers),
    ]);
    ...
    expect(roles.body.data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          label: "Owner",
          canBeInvited: false,
          permissions: expect.arrayContaining(["household.delete"]),
        }),
        expect.objectContaining({ value: "member", canBeInvited: true }),
      ]),
    );
    ...
    expect(permissions.body.data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          value: "creditCards.read",
          label: "View Credit Cards",
          group: "Credit Cards",
        }),
      ]),
    );
