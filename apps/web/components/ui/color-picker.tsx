...
export const ColorPicker = ({
  ...
  useEffect(() => {
    if (value) {
      const [nextHue, nextSaturation, nextLightness] = Color(value).hsl().array();
...
export const ColorPickerEyeDropper = ({ className, ...props }: ColorPickerEyeDropperProps) => {
  ...
  return (
    <Button
      ...
      {...props}
    >
      <HugeiconsIcon icon={DropperIcon} size={16} strokeWidth={2} />
    </Button>
  );
};
...
export const ColorPickerFormat = ({ className, ...props }: ColorPickerFormatProps) => {
  ...
  if (mode === "hex") {
    ...
    return (
      <div
        className={cn("-space-x-px relative flex w-full items-center rounded-md", className)}
        {...props}
      >
        <Input
  ...
  if (mode === "rgb") {
    ...
    const channels = [
      { key: "red", value: rgb[0] },
      { key: "green", value: rgb[1] },
      { key: "blue", value: rgb[2] },
    ];
  ...
  if (mode === "hsl") {
    ...
    const channels = [
      { key: "hue", value: hsl[0] },
      { key: "saturation", value: hsl[1] },
      { key: "lightness", value: hsl[2] },
    ];
