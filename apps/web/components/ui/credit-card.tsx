...
export const CreditCard = ({ className, ...props }: CreditCardProps) => (
  <div
    className={cn(
      "group/credit-card perspective-distant aspect-[8560/5398] w-full max-w-96 text-white",
      "@container",
      className,
    )}
    {...props}
...
export type CreditCardFlipperProps = ButtonHTMLAttributes<HTMLButtonElement>;
  ...
  return (
    <CreditCardFlipContext.Provider value={true}>
      <button
        ...
        type="button"
        className={cn(
          "block h-full w-full border-0 bg-transparent p-0 text-left",
          "@xs:rounded-2xl rounded-lg",
          "transform-3d transition duration-700 ease-in-out",
          supportsHover && "group-hover/credit-card:-rotate-y-180",
          !supportsHover && isFlipped && "-rotate-y-180",
          className,
        )}
        onClick={handleClick}
...
export const CreditCardServiceProvider = ({
  ...
  if (children) {
    return (
      <div className={cn("absolute right-0 bottom-0", "max-h-1/3 max-w-1/3", className)}>
        {children}
      </div>
    );
  }
