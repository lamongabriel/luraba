...
const nonCardShape = {
  ...
  openingBalance: optionalNumber(
    z.number({ error: "Enter a valid opening balance." }).finite(),
  ),
...
export const createEditAccountFormSchema = z
  .discriminatedUnion("type", [
    ...
    z.object({
      ...
      area: optionalNumber(
        z
          .number({ error: "Enter a valid area." })
      ...
      yearBuilt: optionalNumber(
        z
          ...
          .max(
            nextCalendarYear,
            `Year built cannot be after ${nextCalendarYear}.`,
          ),
      ),
    }),
    z.object({
      ...
      year: optionalNumber(
        z
          ...
          .max(
            nextCalendarYear,
            `Vehicle year cannot be after ${nextCalendarYear}.`,
          ),
      ),
...
export type CreateEditAccountFormValues = z.infer<
  typeof createEditAccountFormSchema
>
