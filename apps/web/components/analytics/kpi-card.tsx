...
export function KpiCard({ title, value, icon, description, comparison, trend = [] }: KpiCardProps) {
  ...
  return (
    <Card aria-label={title} className="relative isolate min-h-32 overflow-hidden">
      ...
      <CardContent className="relative z-10">
        <div className="min-w-0 max-w-[60%]">
          ...
          {comparisonText ? (
            <p className={cn("mt-2 text-xs font-medium", trendClasses[state])}>{comparisonText}</p>
          ) : null}
        </div>
      </CardContent>
      {trend.length > 1 ? (
